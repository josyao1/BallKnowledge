...
function ResultName({ name, guessed, correct }: { name: string; guessed: string; correct: boolean }) {
  return (
    <div className={`flex-1 min-w-0 px-2.5 py-1.5 rounded-lg border capcrunch-kicker text-sm font-semibold truncate ${
      correct
...
export function NBAPlayerResultRow({ player, guessed, teamColor = '#4a4a4a', leaders, getters }: Props) {
  ...
  return (
    <div className="flex items-start gap-2 py-1">
      <div
        className="shrink-0 w-9 h-7 flex items-center justify-center rounded-md capcrunch-kicker text-[11px] font-bold tabular-nums mt-0.5"
      ...
      <div className="flex-1 min-w-0">
        ...
        {getters && getters.length > 0 && (
          <div className="pl-1 mt-0.5 capcrunch-kicker text-[9px] text-green-600/70">{getters.join(' · ')}</div>
        )}
      </div>
      <div className="shrink-0 text-right mt-1" style={{ minWidth: 96 }}>
        {leaders
          ...
          : (
            <span className="capcrunch-kicker text-[11px] tabular-nums text-[#555]">
