...
export function NBAScoreboard({ game, compact = false }: Props) {
  ...
  return (
    ...
    >
      ...
      <div className={`relative flex items-center justify-between ${compact ? 'px-4 sm:px-8 py-5' : 'px-6 sm:px-12 py-7'}`}>
        {/* Away */}
        <div className={`flex flex-col items-center flex-1 ${compact ? 'gap-1.5' : 'gap-2'}`}>
          <NBATeamLogo
            abbr={game.away_team}
            className={compact ? 'w-14 h-14 sm:w-20 sm:h-20 object-contain' : 'w-16 h-16 sm:w-24 sm:h-24 object-contain'}
