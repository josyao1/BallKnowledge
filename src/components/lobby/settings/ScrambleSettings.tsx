...
export function ScrambleSettings({ sport, onSportChange, winTarget, onWinTargetChange, careerTo, onCareerToChange, minMpg, onMinMpgChange, minYards, onMinYardsChange, includeDefense, onIncludeDefenseChange }: Props) {
  return (
    <div className="space-y-2.5">
      ...
      {sport === 'nfl' && (
        <Row label="Defense">
          <Chips>
            <Chip active={includeDefense} activeBg={CYAN} activeText="#111" onClick={() => onIncludeDefenseChange(true)}>On</Chip>
            <Chip active={!includeDefense} activeBg={CYAN} activeText="#111" onClick={() => onIncludeDefenseChange(false)}>Off</Chip>
          </Chips>
        </Row>
      )}
    </div>
