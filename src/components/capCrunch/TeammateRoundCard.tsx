...
export function TeammateRoundCard({ pickIndex, size = 'sm' }: Props) {
  ...
  return (
    ...
    >
      <p
        className={`capcrunch-kicker text-white/40 leading-none mb-0.5 ${size === 'lg' ? 'text-[8px] md:text-[10px]' : 'text-[8px]'}`}
      >
        Filter
      </p>
      <p
        className={`capcrunch-title text-[#70BE5B] leading-tight ${size === 'lg' ? 'text-2xl md:text-3xl' : 'text-xl md:text-2xl'}`}
      >
              ...
              >
                <div className="flex items-center justify-between mb-3">
                  <div>
                    ...
                    <p className="capcrunch-title text-sm text-[#70BE5B]">
                      Played with Pick {pickIndex}
                    </p>
                  </div>
                  <button
                ...
                <p className="capcrunch-kicker text-[10px] text-white/60 leading-relaxed">
                  Pick any player who was ever on the same team as Pick {pickIndex} — even in a
                  different year. The overlap just needs to have happened at some point in their
                  careers.
                </p>
              </motion.div>
            </>
