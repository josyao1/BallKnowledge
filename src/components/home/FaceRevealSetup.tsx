...
function ChipRow<T extends number | string>({
  ...
  return (
    <div className="flex flex-col gap-2">
      ...
      <div
        className={`grid gap-2 ${options.length === 4 ? 'grid-cols-4' : options.length === 3 ? 'grid-cols-3' : 'grid-cols-2'}`}
      >
        {options.map((opt) => (
          <button
      ...
      <div className="grid gap-4 lg:grid-cols-[1.2fr_0.8fr] lg:items-start">
        ...
        >
          <div className="p-5 flex flex-col gap-4">
            ...
            <ChipRow
              label="Player active into"
              options={ERA_OPTIONS}
              value={careerTo}
              onChange={setCareerTo}
            />
            <ChipRow
              label="Timer per zoom level"
            ...
            {sport === 'nfl' && (
              <>
                <ChipRow
                  label="Min season yards (offensive)"
                  options={YARDS_OPTIONS}
                  value={minYards}
                  onChange={setMinYards}
                />
                <ChipRow
                  label="Defense pool"
                  options={[
                    { label: 'Well Known', value: 'known' as const },
                    { label: 'All Defense', value: 'all' as const },
                  ]}
                  value={defenseMode}
            ...
            <div className="flex flex-wrap gap-2 justify-center">
              <button
                onClick={() =>
                  navigate('/face-reveal', {
                    state: { sport, careerTo, timer, minYards, minMpg, defenseMode },
                  })
                }
        ...
        <aside className={`${tab !== 'rules' ? 'hidden lg:block' : ''} capcrunch-panel p-5 md:p-6`}>
          ...
          <ul className="text-sm text-white/80 space-y-3 text-left">
            <li>
              ...
              Identify a mystery player from a progressively zoomed-out headshot.
            </li>
            <li>
              <span className="font-bold" style={{ color: COLOR }}>
                Zoom levels:
              </span>{' '}
              Three increasingly wider crop views, then a final hint with initials and team logo.
              Fewer levels seen = more points.
            </li>
            <li>
              <span className="text-white/60 font-bold">Scoring:</span> First correct guess each
              round = 3 pts. All other correct guesses = 1 pt.
            </li>
            <li>
              <span className="text-white/60 font-bold">Timer:</span> Each zoom level has a
              countdown. Runs out and the next level reveals automatically.
            </li>
            <li>
              <span className="text-white/60 font-bold">Skip vote:</span> In multiplayer, players
              can vote to skip ahead to the next zoom level.
            </li>
            <li>
            ...
            <li>
              <span className="text-white/60 font-bold">Era / yards / MPG:</span> Filters narrow the
              player pool — higher thresholds mean more well-known players.
            </li>
            <li>
              <span className="text-white/60 font-bold">Defense pool:</span> NFL only — "Well Known"
              limits defensive players to a curated allowlist of recognizable names.
            </li>
          </ul>
        </aside>
      </div>
