...
function getLogoUrl(abbr: string): string {
...
function bk(side: 'home' | 'away', cat: string, idx: number) { return `${side}_${cat}_${idx}`; }

// Jersey numbers come from parquet as floats — strip ".0"
function cleanJersey(n: string): string {
  if (!n) return '';
  const f = parseFloat(n);
  return isNaN(f) ? n : String(Math.round(f));
}

// Initials hint: "Patrick Mahomes" → "P.M."
...
// ─── Family Feud flip-reveal animation ────────────────────────────────────────

function FlipReveal({ name }: { name: string }) {
  return (
...
function TeamLogo({ abbr, className, imgStyle }: { abbr: string; className?: string; imgStyle?: React.CSSProperties }) {
  return (
    <img src={getLogoUrl(abbr)} alt={abbr} className={className ?? 'w-16 h-16 object-contain'} style={imgStyle}
