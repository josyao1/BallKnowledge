  ...
  if (!careerState || !startersData) {
    return (
      <div className="min-h-screen home-chalkboard flex items-center justify-center">
        <div className="text-white/30 capcrunch-kicker tracking-widest">Loading...</div>
      </div>
    );
  }
          ...
          >
            ...
            {isCorrect ? (
              ...
              <div className="px-4 py-2.5 bg-black/40 border border-white/10">
                <span className="capcrunch-kicker text-sm text-white/30">
            ...
            {/* Team reveal */}
            {correctTeamObj && (
              <div className="px-4 py-4 bg-black/40 border border-white/10 text-center">
                <div className="text-[9px] text-white/30 capcrunch-kicker uppercase tracking-widest mb-1.5">
            ...
            {isHost ? (
              <button
                ...
                className="w-full py-3 capcrunch-title text-xl text-white bg-[#ea580c] hover:bg-[#c2410c] disabled:opacity-50 transition-all"
              >
                {isAdvancing ? 'Loading...' : 'Next Round →'}
              </button>
            ) : (
              <div className="text-center capcrunch-kicker text-sm text-white/30 py-2">
          ...
          <>
            ...
            {!isCorrect && !isLocked && !hasGivenUp && (
              <div className="flex items-center justify-between gap-2">
                ...
                {hintGranted ? (
                  ...
                  <span className="shrink-0 px-2 py-0.5 text-[10px] capcrunch-kicker border border-white/10 text-white/30">
                    {isNBA ? 'PPG' : 'INITIALS'} {hintVotes}/{connectedCount}
                  </span>
                ) : (
                  <button
                    onClick={requestHint}
                    className={`shrink-0 px-2 py-0.5 text-[10px] capcrunch-kicker border transition-all ${
                      hintVotes > 0
              ...
              >
                <div className="flex gap-2">
                  <input
                    ...
                    className="flex-1 bg-black/40 border border-white/10 px-4 py-3 capcrunch-kicker text-sm text-white placeholder-white/20 focus:outline-none focus:border-[#ea580c] transition-colors"
                    autoComplete="off"
                  />
                  <button
            ...
            {!isCorrect && !hasGivenUp && (
              <button
                ...
                className="w-full py-2 capcrunch-kicker text-xs bg-transparent border border-white/10 text-white/25 hover:border-red-900/50 hover:text-red-400/60 transition-all"
              >
                Give Up
              </button>
            )}
            ...
            <AnimatePresence>
              {feedbackMsg && (
                <motion.div
                  ...
                  animate={{ opacity: 1 }}
                  exit={{ opacity: 0 }}
                  className="text-center capcrunch-kicker text-sm text-red-400"
                >
                  {feedbackMsg}
                </motion.div>
              )}
            ...
            {/* Waiting for others */}
            {isCorrect && (
              <div className="text-center capcrunch-kicker text-sm text-white/30">
            ...
            {isAdvancing && (
              <div className="text-center capcrunch-kicker text-sm text-green-400/60">
                Calculating scores...
              </div>
            )}
          </>
