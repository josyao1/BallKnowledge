...
function PassingResultRow({ player, guessed }: { player: BoxScorePassingPlayer; guessed: string }) {
  ...
  return (
    <div className="flex items-center gap-1.5 py-1">
      <span className="capcrunch-kicker text-[10px] text-[#3a3a3a] w-7 text-right shrink-0 tabular-nums">
        {player.number ? `#${player.number}` : '—'}
      </span>
      <ResultName name={player.name} guessed={guessed} correct={correct} />
      <div
        className="capcrunch-kicker text-[10px] text-[#3a3a3a] shrink-0 tabular-nums text-right hidden sm:block"
        style={{ minWidth: 130 }}
      >
      ...
      <div
        className="capcrunch-kicker text-[10px] text-[#3a3a3a] shrink-0 tabular-nums text-right sm:hidden"
        style={{ minWidth: 70 }}
      >
    ...
    <div className="min-h-screen home-chalkboard text-white">
      ...
      <div className="max-w-5xl mx-auto px-3 py-4 space-y-3">
        ...
        <div className="capcrunch-panel p-5 flex flex-col items-center gap-2">
          <div className="capcrunch-kicker text-[10px] text-[#444] tracking-[0.35em] uppercase">
            Final Score
          </div>
          <div className="capcrunch-title text-6xl text-[#FDF100] tabular-nums leading-none">
            {correctRows}
            <span className="text-3xl text-[#444]">/{totalRows}</span>
          </div>
          <div className="capcrunch-kicker text-sm text-[#555]">
            {pct}% ·{' '}
          ...
          {game.spread_line != null && (
            <div
              className={`mt-1 px-3 py-1 rounded border capcrunch-kicker text-xs ${
                spreadCorrect
        ...
        <div className="capcrunch-panel overflow-hidden">
          <div className="flex items-center justify-between px-5 py-4">
            ...
            <div className="flex flex-col items-center gap-1.5 px-3">
              {game.overtime && (
                <span className="px-2 py-0.5 bg-amber-900/30 border border-amber-700/40 rounded capcrunch-kicker text-[9px] text-amber-500 tracking-wider">
                  OT
                </span>
              )}
              <div className="capcrunch-kicker text-[10px] text-[#333] tracking-[0.3em]">FINAL</div>
              <div className="capcrunch-kicker text-[10px] text-[#444] text-center leading-snug">
                {gameLabel}
                <br />
                Wk {game.week}
              </div>
          ...
          <div className="border-t border-white/8 px-4 py-2 flex flex-wrap items-center justify-center gap-x-2 gap-y-0.5">
            ...
            {game.stadium && (
              <span className="capcrunch-kicker text-[10px] text-[#2e2e2e]">· {game.stadium}</span>
            )}
            {game.roof && (
              <span className="capcrunch-kicker text-[10px] text-[#2e2e2e] capitalize">
                · {game.roof}
              </span>
            )}
            {game.temp != null && (
              ...
              <div key={side} className="capcrunch-panel overflow-hidden">
                ...
                <div className="p-3 space-y-3">
                  {data.passing.length > 0 && (
                    <section>
                      <div className="capcrunch-kicker text-[9px] text-white/30 tracking-[0.35em] uppercase mb-1.5 pb-1 border-b border-white/8">
                        Passing
                      </div>
                      {data.passing.map((p, i) => (
                        <PassingResultRow
                          key={`${p.id}-${i}`}
                          player={p}
                          guessed={guesses[bk(side, 'passing', i)] ?? ''}
                        />
                      ))}
                    </section>
                  )}
                    ...
                    <section>
                      ...
                      {data.rushing.map((p, i) => (
                        <RushingResultRow
                          key={`${p.id}-${i}`}
                          player={p}
                          guessed={guesses[bk(side, 'rushing', i)] ?? ''}
                        />
                      ))}
                    </section>
                  )}
                    ...
                    <section>
                      ...
                      {data.receiving.map((p, i) => (
                        <ReceivingResultRow
                          key={`${p.id}-${i}`}
                          player={p}
                          guessed={guesses[bk(side, 'receiving', i)] ?? ''}
                        />
                      ))}
                    </section>
                  )}
