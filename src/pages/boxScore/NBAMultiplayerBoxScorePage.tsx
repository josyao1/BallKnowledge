...
export function NBAMultiplayerBoxScorePage() {
  ...
  useEffect(() => {
    ...
    loadNBABoxScoreYear(careerState.season)
      .then((games) => {
        ...
        setGame(g);
        getNBASeasonPlayerPool(g.season)
  ...
  useEffect(() => {
    if (lobby?.status === 'finished' && gameRef.current) {
      navigate(`/lobby/${code}/nba-box-score/results`, {
        state: { game: gameRef.current, myGuesses: guessesRef.current },
      });
    }
  }, [lobby?.status]);
  ...
  if (game) {
    for (const side of ['home', 'away'] as const) {
      ...
      game.box_score[side].forEach((p, i) => {
        if (isCorrect(side, i, p.name)) correctCount++;
      });
    }
  }
        ...
        {finished && (
          ...
          >
            <span className="capcrunch-kicker text-sm text-amber-400 font-semibold">
              ⏱ Time's up! Score: {correctCount} — waiting for others...
            </span>
          </motion.div>
        )}
              ...
              >
                ...
                >
                  ...
                  <div className="ml-auto">
                    <div
                      className="capcrunch-kicker text-[10px] tracking-wider"
                      style={{ color: sideDone === players_list.length ? '#4ade80' : '#555' }}
                    >
                ...
                <div className="p-3">
                  ...
                  {players_list.map((p, i) => {
                    ...
                    return (
                      ...
                      >
                        <div
                          className="relative shrink-0 w-9 h-7 flex items-center justify-center rounded-md capcrunch-kicker text-[11px] font-bold tabular-nums"
                          style={{
                            background: `${color}30`,
                            color,
