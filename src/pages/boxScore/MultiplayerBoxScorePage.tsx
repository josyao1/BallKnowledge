...
export function MultiplayerBoxScorePage() {
  ...
  useEffect(() => {
    spreadGuessRef.current = spreadGuess;
  }, [spreadGuess]);
  ...
  const handleFinish = useCallback(() => {
    ...
    const guessedArr: string[] = myHintRequestedRef.current
      ? [...entries, 'HINT_REQUEST']
      : [...entries];
  ...
  function confirmCandidate(name: string) {
    ...
    setTimeout(
      () =>
        rowRefs.current[matches[0].key]?.scrollIntoView({ behavior: 'smooth', block: 'nearest' }),
      50,
    );
    searchRef.current?.focus();
    ...
    const sg = spreadGuessRef.current;
    const spreadOk =
      gameRef.current?.spread_line != null &&
      sg !== '' &&
  ...
  const myHintRequested =
    myHintRequestedLocal ||
    (players.find((p) => p.player_id === currentPlayerId)?.guessed_players || []).includes(
      'HINT_REQUEST',
    );
  const hintVotes = players.filter((p) =>
    (p.guessed_players || []).includes('HINT_REQUEST'),
  ).length;
  ...
  if (game) {
    for (const side of ['home', 'away'] as const)
      for (const cat of ['passing', 'rushing', 'receiving'] as const) {
        ...
        rows.forEach((p, i) => {
          if (isCorrect(side, cat, i, p.name)) correctCount++;
        });
      }
  }
  const spreadOk =
    ...
    return (
      <div className="min-h-screen bg-[#080808] flex items-center justify-center">
        <div className="flex flex-col items-center gap-4">
          ...
          <span className="capcrunch-kicker text-[#666] tracking-[0.4em] text-xs">
            LOADING GAME
          </span>
        </div>
      </div>
  ...
  return (
    <div className="min-h-screen text-white" style={{ background: '#080808' }}>
      ...
      <header className="sticky top-0 z-30 capcrunch-panel border-b border-white/10">
        <div className="max-w-5xl mx-auto px-3 py-2.5 flex items-center gap-2">
          ...
          <div className="shrink-0 min-w-[56px] text-center">
            <div
              className="capcrunch-title text-2xl tabular-nums leading-none"
          ...
          <div className="flex-1 text-center">
            <span className="capcrunch-title text-lg" style={{ color: '#FDF100' }}>
              BOX SCORE
            </span>
          </div>
          ...
          <div className="flex items-center gap-2 shrink-0">
            ...
            <div className="flex items-center gap-1 px-2.5 py-1 border border-white/10">
              <span className="capcrunch-title text-xl tabular-nums" style={{ color: '#FDF100' }}>
                {correctCount}
              </span>
              <span className="capcrunch-kicker text-xs text-[#444]">/{totalRows}</span>
              {finished && spreadOk && (
                <span className="capcrunch-kicker text-[10px] text-green-400 ml-0.5">+S</span>
              )}
            </div>
          </div>
        </div>
        ...
        <div className="border-t border-white/4 px-3 py-1.5 flex gap-1.5 overflow-x-auto">
          {players.map((p) => {
            ...
            return (
              <div
                key={p.player_id}
                className={`flex items-center gap-1 shrink-0 px-2 py-1 rounded capcrunch-kicker text-[10px] ${
                  done && score > 0
          ...
          <div className="shrink-0 capcrunch-kicker text-[10px] text-[#444] self-center ml-auto whitespace-nowrap">
              ...
              {showDropdown && candidates.length > 0 && (
                ...
                >
                  {candidates.map((c, i) => (
                    ...
                    >
                      <span className="capcrunch-kicker text-sm text-white font-semibold flex-1 min-w-0 truncate">
                        {c.name}
                      </span>
                      <svg
      ...
      <div className="max-w-5xl mx-auto px-3 py-4 space-y-4">
        ...
        >
          <div
            className="absolute -left-16 top-1/2 -translate-y-1/2 w-40 h-40 rounded-full blur-3xl pointer-events-none"
            style={{ background: awayColor, opacity: 0.18 }}
          />
          <div
            className="absolute -right-16 top-1/2 -translate-y-1/2 w-40 h-40 rounded-full blur-3xl pointer-events-none"
          ...
          <div className="relative flex items-center justify-between px-4 sm:px-8 py-5">
            <div className="flex flex-col items-center gap-1.5 flex-1">
              ...
              />
              <span
                className="capcrunch-title text-xl sm:text-3xl leading-none"
                style={{ color: awayColor }}
              >
                {game.away_team}
              </span>
              <span className="capcrunch-title text-5xl sm:text-6xl text-white leading-none tabular-nums">
                {game.away_score}
              </span>
              <span className="capcrunch-kicker text-[9px] text-[#666] tracking-widest uppercase">
                Away
              </span>
            </div>
            <div className="flex flex-col items-center gap-1.5 px-2">
              {game.overtime && (
                <span
                  className="px-2 py-0.5 rounded-full capcrunch-kicker text-[10px]"
                  style={{
              ...
              <div className="capcrunch-kicker text-[11px] text-[#444] tracking-[0.4em]">FINAL</div>
              <div className="capcrunch-kicker text-[10px] text-[#555] text-center leading-snug">
                {GAME_TYPE_LABELS[game.game_type] ?? game.game_type}
                <br />
                WK {game.week}
              </div>
              <div className="capcrunch-kicker text-[9px] text-[#444]">{game.season}</div>
            </div>
            <div className="flex flex-col items-center gap-1.5 flex-1">
              <TeamLogo
                abbr={game.home_team}
        ...
        {!finished && (
          ...
          >
            {hintsGranted ? (
              <span className="capcrunch-kicker text-xs text-amber-400 font-semibold">
                💡 Hints active — player initials shown
              </span>
            ) : (
              <>
                <span className="capcrunch-kicker text-xs text-[#555]">
        ...
        {finished && (
          ...
          >
            <span className="capcrunch-kicker text-sm text-amber-400 font-semibold">
                    ...
                    {(['passing', 'rushing', 'receiving'] as const).map((cat) => {
                      ...
                      return rows.length > 0 ? (
                        <div key={cat} className="text-center">
                          <div
                            className="capcrunch-kicker text-[8px] tracking-wider"
                            style={{ color: done === rows.length ? '#4ade80' : '#555' }}
                          >
                            {done}/{rows.length}
                          </div>
                          <div className="capcrunch-kicker text-[7px] text-[#3a3a3a] uppercase">
                            {cat.slice(0, 3)}
                          </div>
                        </div>
                      ) : null;
                        ...
                        {rows.map((p, i) => {
                          ...
                          return (
                            ...
                            >
                              ...
                              <div
                                className="relative shrink-0 w-9 h-7 flex items-center justify-center capcrunch-kicker text-[11px] font-bold tabular-nums"
                                style={{
                                  background: `${color}30`,
                                  color,
                              ...
                              >
                                {jersey ? `#${jersey}` : '–'}
                                {tmCount > 0 && (
                                  <span className="absolute -top-1.5 -right-1.5 w-4 h-4 rounded-full bg-blue-500 border border-[#080808] flex items-center justify-center capcrunch-kicker text-[8px] text-white font-bold">
                                    {tmCount}
                                  </span>
                                )}
                              </div>
                                ...
                                <AnimatePresence mode="wait">
                                  {correct ? (
                                    <motion.div
                                      ...
                                      style={{
                                        background:
                                          'linear-gradient(90deg, rgba(22,163,74,0.25) 0%, rgba(22,163,74,0.08) 100%)',
                                    ...
                                    >
                                      ...
                                      <span className="capcrunch-kicker text-sm font-semibold text-green-300 truncate">
                                    ...
                                    >
                                      <span className="capcrunch-kicker text-sm text-red-400/80">
                                        {p.name}
                                      </span>
                                    </motion.div>
                                  ) : (
                                    <div className="flex flex-col gap-0.5">
                                      <div className="px-2.5 py-1.5 border border-white/8 bg-black/30">
                                        <span className="capcrunch-kicker text-sm text-[#2a2a2a] select-none">
                                          ████████
                                        </span>
                                      </div>
                                      {hintsGranted && (
                                        <div
                                          className="pl-1 capcrunch-kicker text-[11px] tracking-widest font-mono"
                              ...
                              <div className="shrink-0 text-right" style={{ minWidth: 72 }}>
                                {cat === 'passing' && (
                                  <span className="capcrunch-kicker text-[11px] tabular-nums">
                                    ...
                                    <span
                                      className={
                                        p.tds > 0 ? 'text-green-400 font-bold' : 'text-[#666]'
                                      }
                                    >
                                      {p.tds}TD
                                    </span>
                                    {p.ints > 0 && (
                                      <span className="text-red-400"> {p.ints}INT</span>
                                    )}
                                  </span>
                                )}
                                {cat === 'rushing' && (
            ...
            <div className="flex items-center gap-3 flex-wrap mt-2">
              <input
                ...
                onChange={(e) => {
                  ...
                  const guessedArr = myHintRequestedRef.current
                    ? [...entries, 'HINT_REQUEST']
                    : entries;
                  updatePlayerScore(lobby.id, score, boxCount, guessedArr, [], false);
                }}
                ...
                className="w-32 bg-black/40 border border-white/15 px-3 py-2 capcrunch-kicker text-sm text-white placeholder-[#2a2a2a] focus:outline-none focus:border-[#FDF100]/40 disabled:opacity-40 transition-colors"
              />
              <span className="capcrunch-kicker text-xs text-[#555]">+ = home favored</span>
              {finished && spreadOk && (
                <span className="capcrunch-kicker text-xs text-green-400 font-semibold">
                  ✓ correct!
                </span>
              )}
            </div>
        ...
        <div className="pb-14">
          {!finished ? (
            ...
            <div className="text-center capcrunch-kicker text-sm text-[#555] tracking-widest">
              Waiting for {players.length - doneCount} more player
              {players.length - doneCount !== 1 ? 's' : ''}...
            </div>
          )}
        </div>
      </div>
