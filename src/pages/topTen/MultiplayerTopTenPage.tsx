        ...
        {phase === 'suspense' ? (
          ...
          >
            <motion.p
              ...
              animate={{
                textShadow: [
                  '0 0 0px rgba(112,190,91,0)',
                  '0 0 24px rgba(112,190,91,0.5)',
                  '0 0 0px rgba(112,190,91,0)',
                ],
              }}
...
export function MultiplayerTopTenPage() {
  ...
  useEffect(() => {
    ...
    findLobbyByCode(code)
      .then((result) => {
        ...
        getLobbyPlayers(result.lobby.id)
          .then((pr) => {
  ...
  const showInitialsHint: boolean =
    isDivisionRound || isTeamRound ? hintLevel >= 1 : hintLevel >= 2;
  ...
  useEffect(() => {
    const anim = cs.reveal_anim as
      | { guessedName: string; isCorrect: boolean; endsAt: string; playerId?: string }
      | null
      | undefined;
    if (!anim?.endsAt) {
    ...
    setRevealOverlay({
      guessedName: anim.guessedName,
      isCorrect: anim.isCorrect,
      playerId: anim.playerId,
      sport: (cs.sport || 'nba') as 'nba' | 'nfl',
    });
    ...
    async (
      ...
      try {
        ...
        if (raceMode) {
          ...
          const rebuttalComplete =
            newRebuttalTriggerIdx !== null && nextIndex === newRebuttalTriggerIdx;
          ...
          const buildAnim = (name: string, pid?: string) => {
            ...
            return {
              anim: { guessedName: name, isCorrect: correct, endsAt, playerId: pid },
              deadline: new Date(new Date(endsAt).getTime() + turnTimerSecs * 1000).toISOString(),
            };
          };

          if (roundOver) {
            const mergedIdxAttr =
              (extras.guess_index_attribution as Record<string, number[]>) ??
              (cs.guess_index_attribution || {});
            const winnerIds = determineWinners(
              ...
              false,
              '',
              computeHardnessScores(mergedIdxAttr),
            );
            const newState = {
              ...cs,
              ...extras,
          ...
          const allRemainingOnLastStrike =
            maxStrikes > 1 &&
            newActivePlayers.length > 0 &&
            newActivePlayers.every((id) => (currentStrikes[id] || 0) >= maxStrikes - 1);
          const isOneOnOne = turnOrder.length === 2;
          const lastStanding =
            !isOneOnOne && newActivePlayers.length === 1 && !allRemainingOnLastStrike;

          const roundOver = allSlotsFilled || lastStanding || allOut;

          const currentWasEliminated =
            !eliminated.includes(currentTurnId) && currentEliminated.includes(currentTurnId);
          const nextIndex =
            newActivePlayers.length > 0
              ? (currentWasEliminated ? currentTurnIndex : currentTurnIndex + 1) %
                newActivePlayers.length
              : 0;
          ...
          if (roundOver) {
            ...
            if (guessedName) {
              ...
              newDeadline = new Date(
                new Date(animEndsAt).getTime() + turnTimerSecs * 1000,
              ).toISOString();
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    if (matched.length > 0) {
      ...
      const newAttribution = {
        ...
        [currentPlayerId ?? '']:
          ((cs.guess_attribution || {})[currentPlayerId ?? ''] || 0) + matched.length,
      };
      ...
      await advanceTurn(
        true,
        newGuessedIndices,
        { guess_attribution: newAttribution, guess_index_attribution: newIndexAttribution },
        entries[matched[0]].playerName,
        String(entries[matched[0]].playerId),
      );
    } else {
  ...
  async function handleForceEnd() {
    ...
    try {
      ...
      const winnerIds = determineWinners(
        ...
        playerStrikes,
        maxStrikes,
        !raceMode && activePlayers.length === 1,
        activePlayers[0] || '',
        hardness,
      );
    ...
    try {
      ...
      if (newVotes.length >= turnOrder.length) {
        ...
        const winnerIds = determineWinners(
          ...
          false,
          '',
          computeHardnessScores(guessIndexAttribution),
        );
        const newState = {
          ...cs,
          give_up_votes: newVotes,
          winner_ids: winnerIds,
          winner_id: winnerIds[0] || '',
          reveal_anim: null,
        };
  ...
  async function handleSkipCategory() {
    ...
    try {
      const {
        ...
        isDivisionRound: newDiv,
        isTeamRound: newTeam,
        isSingleSeason: newSingleSeason,
        teamAbbr: newTeamAbbr,
      } = await generateRound();
  ...
  const timerColor =
    displayTimerFraction > 0.5 ? '#70BE5B' : displayTimerFraction > 0.25 ? '#f59e0b' : '#ef4444';
  ...
  return (
    <div className="min-h-screen home-chalkboard text-white flex flex-col">
      ...
      <header className="px-3 py-3 border-b border-white/10 flex items-center gap-2 bg-black/20 relative z-20">
        ...
        <span className="capcrunch-kicker text-[9px] text-white/25 tracking-[0.2em] hidden sm:inline shrink-0">
          {sport.toUpperCase()}
        </span>
        <div className="ml-auto flex items-center gap-1 sm:gap-2 min-w-0">
          ...
          {hintLevel < maxHintLevel && (
            ...
            >
              Hint {hintLevel + 1}
              {hintVoteCount > 0 ? ` ${hintVoteCount}/${players.length}` : ''}
            </button>
          )}
          {isHost && !isMyTurn && (
            <button
      ...
      <main className="flex-1 max-w-lg mx-auto w-full px-4 pb-8 flex flex-col gap-4">
            ...
            return (
              <div
                ...
                className={`flex items-center gap-1.5 px-2 py-1 border transition-colors ${
                  isElim
                    ? 'opacity-20 border-white/5 bg-transparent'
                    : isCurrent
                      ? 'border-[#70BE5B]/40 bg-[#70BE5B]/5'
                      : isMe
              ...
              >
                ...
                <span
                  className={`capcrunch-kicker text-[9px] max-w-[56px] truncate ${
                ...
                {raceMode ? (
                  <span
                    className={`capcrunch-title text-[10px] tabular-nums ${pGuesses >= raceTarget ? 'text-[#E2008A]' : 'text-white/50'}`}
                  >
                    {pGuesses}/{raceTarget}
                  </span>
                ) : (
                  <span className="capcrunch-title text-[10px] tracking-tight">
                    {Array.from({ length: maxStrikes }).map((_, i) => (
                      <span key={i} className={i < pStrikes ? 'text-red-400' : 'text-white/12'}>
                        ✕
                      </span>
                    ))}
                  </span>
        ...
        {raceMode && rebuttalTriggerIndex !== null && (
          ...
          >
            <p className="capcrunch-kicker text-[10px] text-[#E2008A] tracking-[0.25em]">
              REBUTTAL — everyone gets one last turn
            </p>
          </motion.div>
        )}
        ...
        <motion.div
          ...
          transition={
            isMyTurn ? { duration: 2, repeat: Infinity, ease: 'easeInOut' } : { duration: 0.3 }
          }
        >
          <div className="flex-1">
            <p
              className={`capcrunch-kicker tracking-[0.25em] mb-1 transition-all ${
          ...
          <span className="capcrunch-title text-2xl tabular-nums" style={{ color: timerColor }}>
            {displayTimeLeft}
          </span>
        </motion.div>
        ...
        <div className="capcrunch-panel p-3">
          <div className="flex items-center justify-between mb-2">
            ...
            {raceMode && (
              <p className="capcrunch-kicker text-[9px] text-[#E2008A]/60 tracking-[0.2em]">
                First to {raceTarget}
              </p>
            )}
          </div>
            ...
            {playersInTurnOrder.map((p) => {
              ...
              return (
                ...
                >
                  ...
                  <span
                    className={`capcrunch-kicker text-sm flex-1 truncate ${
                  ...
                  {raceMode ? (
                    <div className="flex items-center gap-1.5">
                      {Array.from({ length: raceTarget }).map((_, i) => (
                        <span
                          key={i}
                          className={`block w-2 h-2 rounded-full ${i < guesses ? 'bg-[#E2008A]' : 'bg-white/10'}`}
                        />
                      ))}
                    </div>
                  ) : (
                    <span className="capcrunch-title text-[10px]">
                      {Array.from({ length: maxStrikes }).map((_, i) => (
                        <span
                          key={i}
                          className={i < strikes ? 'text-red-400' : 'text-white/12 opacity-20'}
                        >
                          ✕
                        </span>
                      ))}
                  ...
                  <span className="capcrunch-kicker text-[10px] text-[#70BE5B] w-8 text-right tabular-nums">
                    {guesses} pts
                  </span>
                </div>
              );
