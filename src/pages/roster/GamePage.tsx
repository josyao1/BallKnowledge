...
export function GamePage() {
    ...
    if (lastSyncRef.current.score !== currentScore || lastSyncRef.current.count !== currentCount) {
      ...
      const timeout = setTimeout(() => {
        syncScore(
          currentScore,
          currentCount,
          guessedPlayers.map((p) => p.name),
          incorrectGuesses,
          false,
          lobbyIdRef.current || undefined,
        );
      }, 300);
    ...
    (async () => {
      try {
        ...
        if (saved.sport === 'nba') {
          ...
          rosterPlayers =
            (await fetchStaticNFLRoster(team.abbreviation, parseInt(saved.season))) ?? [];
  ...
  useEffect(() => {
    if (status === 'idle' && selectedTeam) startGame();
  ...
  useEffect(() => {
    if (timeRemaining <= 0 && status === 'playing') endGame();
  ...
  return (
    ...
    >
      ...
      <header
        className={`relative z-10 p-6 capcrunch-panel border-b transition-colors duration-300 ${isShotClockActive ? 'border-red-500/60' : 'border-white/10'}`}
      >
        <div className="max-w-7xl mx-auto flex flex-col md:flex-row justify-between items-center gap-8">
          <div className="flex items-center gap-8">
            {divisionTeams.length > 0 && lobbyDivisionConference && lobbyDivisionName ? (
              <div className="flex flex-col">
                ...
                <div className="flex gap-1.5 mt-1">
                  {divisionTeams.map((abbr) => (
                    <span
                      key={abbr}
                      className="text-[9px] text-white/50 capcrunch-kicker px-1.5 py-0.5 bg-white/5"
                    >
                      {abbr}
                    </span>
                  ))}
              ...
              <TeamDisplay
                ...
                record={showSeasonHints ? teamRecord : null}
                sport={sport}
              />
            )}
            ...
            <div className="flex flex-col items-center">
              <span className="capcrunch-kicker text-[9px] text-white/40 tracking-[0.4em] uppercase mb-1">
                Clock
              </span>
              <div
                className={`capcrunch-title text-3xl transition-colors duration-300 ${isShotClockActive ? 'text-red-500 animate-pulse' : 'text-white'}`}
              >
          ...
          <div className="flex gap-4">
            <div className="capcrunch-panel px-6 py-3 text-center min-w-[100px]">
              <div className="capcrunch-kicker text-[9px] text-white/40 tracking-[0.3em] uppercase mb-1">
                Correct
              </div>
              {hideResultsDuringGame ? (
            ...
            <div className="capcrunch-panel px-6 py-3 text-center min-w-[100px]">
              ...
              <div className="capcrunch-title text-3xl text-white/40">{currentRoster.length}</div>
            </div>
          </div>
        </div>
      </header>
        ...
        {isMultiplayer && players.length > 0 && (
          <aside className="w-full md:w-72 flex-shrink-0">
            <div className="capcrunch-panel p-4 h-full">
              <h3 className="capcrunch-kicker text-[9px] text-white/40 mb-6 text-center">
                Live Pit
              </h3>
              <LiveScoreboard
                players={players}
