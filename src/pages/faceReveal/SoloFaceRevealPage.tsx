...
export function SoloFaceRevealPage() {
  ...
  useEffect(() => {
    async function loadPool() {
      if (sport === 'nba') {
        ...
        if (minMpg)
          filtered = filtered.filter((p) => p.seasons.some((s) => (s.min ?? 0) >= minMpg));
        setPool(
          filtered.map((p) => ({
  ...
  const pickNext = useCallback(
    (currentPool: PlayerEntry[]) => {
      ...
      setFocalPoint({
        x: Math.round(38 + Math.random() * 24),
        y: Math.round(20 + Math.random() * 16),
      });
      setPlayer(chosen);
  ...
  return (
    <div className="min-h-screen home-chalkboard text-white flex flex-col">
      ...
      {/* Header */}
      <header
        className="capcrunch-panel m-4 mb-2 p-3 flex items-center gap-3"
          ...
          <>
            ...
            <div className="w-full max-w-sm">
              ...
              <div className="flex items-center justify-between mt-1">
                ...
                <span className="capcrunch-kicker text-[9px]" style={{ color }}>
                  {countdown}s
                </span>
              </div>
            </div>
            ...
            <div className="flex flex-col items-center gap-2">
              <motion.div
                ...
                animate={{
                  opacity: 1,
                  scale: 1,
                  boxShadow: status === 'correct' ? '0 0 0 4px #22c55e' : `0 0 0 2px ${COLOR}4d`,
                }}
                transition={{ duration: 0.3 }}
              ...
              <AnimatePresence>
                {player && status === 'active' && zoomLevel === 4 && (
                  ...
                  >
                    <div
                      className="capcrunch-title text-4xl tracking-widest"
                      style={{ color: COLOR }}
                    >
                    ...
                    {player.longestTeam && (
                      <>
                        ...
                        <div className="flex flex-col items-center gap-1">
                          ...
                          <div className="capcrunch-kicker text-[8px] text-white/30 tracking-[0.2em]">
                            LONGEST TEAM
                          </div>
                        </div>
                      </>
            ...
            <AnimatePresence>
              {(status === 'correct' || status === 'revealed') && player && (
                ...
                >
                  ...
                  <div className="capcrunch-title text-3xl text-[#d4af37]">
                    {player.player_name}
                  </div>
                  {status === 'revealed' && (
                    <p className="capcrunch-kicker text-[9px] text-white/30 mt-2">
            ...
            <AnimatePresence>
              {feedbackMsg && (
                <motion.div
                  ...
                  animate={{ opacity: 1 }}
                  exit={{ opacity: 0 }}
                  className="capcrunch-kicker text-xs text-red-400"
                >
                  {feedbackMsg}
                </motion.div>
              )}
                  ...
                  <div className="flex-1 relative">
                    <input
                      ...
                      onChange={(e) => {
                        ...
                        setSuggestions(getSuggestions(e.target.value, pool));
                      }}
                      onKeyDown={(e) => {
                    ...
                    {suggestions.length > 0 && (
                      <div
                        className="absolute left-0 right-0 top-full mt-1 z-20 bg-black/95 border overflow-hidden shadow-xl"
                        style={{ borderColor: `${COLOR}40` }}
                      >
                        {suggestions.map((s) => (
                          <button
