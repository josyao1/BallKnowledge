...
export function SoloCapCrunchPage() {
  ...
  useEffect(() => {
    const autoStartState = location.state as { autoStart?: boolean; selectedSport?: Sport; statCategory?: StatCategory | null; totalRounds?: number } | null;
    if (!autoStartState?.autoStart || phase !== 'sport-select' || consumedAutoStartRef.current) return;
  ...
  const handleConfirmYear = async () => {
    ...
    try {
      ...
      if (updated.selectedPlayers.length === totalRounds) {
        ...
        const nextTeam = assignRandomTeam(selectedSport, statCategory, usedTeamsRef.current, usedSpecialTypes, updated.selectedPlayers, updated.selectedPlayers.length === totalRounds - 1);
  ...
  useEffect(() => {
    ...
    findOptimalLastPick(selectedSport, lastPick.team, statCategory, remainingBudget, lastPick.isBust ? 0 : lastPick.statValue, undefined, hwFilter, picks)
      ...
      <div className="min-h-screen capcrunch-shell text-white flex flex-col overflow-hidden relative">
        ...
        <header className="relative z-10 p-6 border-b border-white/10 capcrunch-panel">
          <div className="max-w-7xl mx-auto flex justify-between items-center">
            ...
            <button onClick={() => navigate('/')} className="px-4 py-2 capcrunch-btn-secondary capcrunch-title text-sm transition">Back</button>
          </div>
        </header>
            ...
            <div className="grid gap-4 lg:grid-cols-[1.2fr_0.8fr]">
              ...
              >
                ...
                <ul className="text-sm text-white/80 space-y-2.5 text-left">
                  ...
                  <li><span className="text-white/60 font-bold">Special rounds:</span> Filters can be a division, a college + pro conference combo, a teammate round, or more.</li>
          ...
          <div className="absolute inset-0 z-20 flex items-center justify-center bg-black/55 backdrop-blur-sm p-6">
            ...
            >
              <div className="mb-8">
                ...
                <p className="capcrunch-body text-white/60 text-sm md:text-base">Choose a league and then the settings screen underneath will unlock.</p>
              </div>
                  ...
                  <div className="flex h-full flex-col justify-between">
                    <div>
                      ...
                      <p className="capcrunch-body mt-3 max-w-sm text-sm text-white/65">
                        Per-game stats, team career games played, and a quicker high-volume scoring style.
                      </p>
                    </div>
                ...
                >
                  ...
                  <div className="flex h-full flex-col justify-between">
                    <div>
                      ...
                      <p className="capcrunch-body mt-3 max-w-sm text-sm text-white/65">
                        Season stats plus career stat variants with broader qualifier and special-round combinations.
                      </p>
                    </div>
                    <span className="capcrunch-kicker text-white/70 group-hover:text-white">Continue</span>
                  </div>
      ...
      >
          ...
          >
            ...
            >
              <motion.div
                ...
                style={{ textShadow: '0 0 40px rgba(253,241,0,0.55), 0 0 100px rgba(226,0,138,0.28)' }}
              >
                EXACT!
              </motion.div>
              <div className="capcrunch-kicker text-xl md:text-2xl text-white mb-1">Perfect score</div>
        ...
        <main className="relative z-10 flex-1 w-full px-2 sm:px-3 md:px-6 py-2 md:py-4 flex flex-col lg:flex-row gap-2 md:gap-4 overflow-hidden">
                ...
                {!selectedPlayerName ? (
                  ...
                  <div className="space-y-2 md:space-y-4 flex flex-col flex-1">
                    <div className="p-2 md:p-4 bg-white/[0.03] border border-white/10">
                      <p className="capcrunch-title text-xs md:text-lg text-white truncate">{selectedPlayerName}</p>
          ...
          <div className="w-full lg:w-80 lg:flex-shrink-0 flex flex-col min-h-[220px] lg:min-h-0">
            ...
            >
                <h3 className="capcrunch-title text-xs md:text-lg text-[#FDF100] mb-2 md:mb-4">Your Lineup</h3>
          ...
          <main className="flex-1 flex flex-col lg:flex-row gap-6 min-h-0">
            ...
            <div className="lg:w-1/3 flex flex-col gap-4">
              ...
              <div className="flex flex-col gap-2 md:gap-3">
                ...
                >
                  <span className="relative z-10 capcrunch-title text-base md:text-lg text-white/70 tracking-widest">Exit Game</span>
                </button>
              </div>
            </div>
                    ...
                    >
                      <div className="flex justify-between items-center relative z-10 gap-2">
                        <div className="flex items-center gap-2 min-w-0 flex-1">
                          ...
                          {isBust && (
                            <div className="text-[9px] text-red-400/70 mt-0.5">busted by {fmt(totalBefore + player.statValue - targetCap)}</div>
                          )}
                          {isNotOnTeam && (
