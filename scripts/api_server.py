...
# Static career pool loaded from scripts/data/nba_careers.json at startup.
# List of full player dicts; dict keyed by player_id for O(1) lookup.
_nba_static_careers: list[dict] = []
_nba_static_by_id:   dict[int, dict] = {}


def _load_nba_career_data() -> None:
    """Load pre-built career data from the static JSON file."""
    global _nba_static_careers, _nba_static_by_id
    data_path = Path(__file__).parent / "data" / "nba_careers.json"
    if not data_path.exists():
        print(
            "Career mode: nba_careers.json not found.\n"
            "  Run:  python scripts/generate_nba_careers.py"
        )
        return
    with open(data_path) as f:
        _nba_static_careers = json.load(f)
    _nba_static_by_id = {p["player_id"]: p for p in _nba_static_careers}
    print(f"Career mode: loaded {len(_nba_static_careers)} NBA players from static data")


_load_nba_career_data()


def _career_start_year(p: dict) -> int:
    """Return the first season year for a player (e.g. '2003-04' → 2003)."""
    seasons = p.get("seasons", [])
    years = [int(s["season"][:4]) for s in seasons if s.get("season")]
    return min(years) if years else 0


def _career_end_year(p: dict) -> int:
    """Return the last season year for a player (e.g. '2020-21' → 2020)."""
    seasons = p.get("seasons", [])
    years = [int(s["season"][:4]) for s in seasons if s.get("season")]
...
async def get_random_career_player(
    ...
    """Return a random NBA player from the pre-built static career pool.
    ...
    """
    import random
    if not _nba_static_careers:
        raise HTTPException(
            status_code=503,
            detail="NBA career data not loaded — run: python scripts/generate_nba_careers.py",
        )
    pool = _nba_static_careers
    if career_from:
        pool = [p for p in pool if _career_start_year(p) >= career_from]
    if career_to:
        pool = [p for p in pool if _career_end_year(p) >= career_to]
    if not pool:
        raise HTTPException(status_code=404, detail="No eligible players found for the given era filter")
    chosen = random.choice(pool)
    return {"player_id": chosen["player_id"], "player_name": chosen["player_name"]}


@app.get("/career/{player_id}")
async def get_career_stats(player_id: int):
    """Return full career stats for an NBA player from the pre-built static pool."""
    player = _nba_static_by_id.get(player_id)
    if not player:
